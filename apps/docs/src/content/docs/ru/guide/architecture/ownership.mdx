---
title: Владение
description: "Два провайдера, одно условие — что значит last-write-wins на практике, как переопределить в тестах и почему дефолт именно такой."
---
...
<Aside type="caution" title="Семантическое изменение в v0.10">
v0.9 держал стек на пару `(trigger, name)` с откатом к предыдущей регистрации при unregister. v0.10 упростила это до одного слота — предыдущая регистрация теряется. Если какой-то use case полагался на stack-fallback (редко), см. паттерн "Pick one provider" ниже или закрепи одну каноничную регистрацию.
</Aside>
...
<Aside type="tip">
Цикл mount → cleanup → mount в StrictMode заканчивается записью второго монтирования: первый монт пишет, cleanup первого монта удаляет эту запись (она ещё live в тот момент), и второй монт записывает снова. Политика структурная — никаких эвристик про double-mount.
</Aside>
