...
Builder-API уже доступен в v0.10 — `import { createTrigger } from '@triggery/core/builder'` и пиши `createTrigger<S>().require('user').handle(...)`, чтобы сузить автоматически. Гард в рантайме идентичен; меняется только типизация.
</Aside>
