...
Также два семантических изменения:
...
Если код полагался на last-mount-wins для actions — переключи call site на
`runtime.registerAction(trigger.id, 'x', fn)`. Этот путь не изменился и
по-прежнему last-mount-wins.
...
### 5. (Опционально) Переключи inspector на factory pattern
