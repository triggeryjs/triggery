...
The single inline-generic captures the whole port surface — three maps for events, conditions and actions. The handler `ctx` is now strongly typed: try to fire a wrong event name or pass the wrong action payload and TypeScript stops you.
