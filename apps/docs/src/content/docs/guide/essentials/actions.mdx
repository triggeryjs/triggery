...
Like conditions, actions register on an internal stack. The most recently mounted reactor's handler wins; on unmount, the previous one takes over. In DEV, the runtime emits a one-time warning per `(triggerId, actionName)` when a second live registration arrives:
...
1. **Overrides are first-class.** A modal mounts and wants to intercept `showToast`? Mount the override, it wins, on close it unmounts and the global handler takes over again.
