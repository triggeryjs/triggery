...
description: "React's StrictMode double-invokes useEffect to flush out cleanup bugs. Triggery's last-mount-wins stack is safe by construction — and Solid / Vue dev-mode double-renders behave the same way."
---
...
```ts title="What the runtime stack looks like"
{
...
|---|---|
| React StrictMode mount → unmount → mount | The cleanup runs between, the stack is empty, the second mount pushes one registration. One live registration. |
| Two providers for the same condition | Last-mount-wins on the stack. Unmounting the second pops back to the first. DEV warning once. |
...
| HMR on a trigger file | Trigger replaced atomically; existing condition/action stacks survive; inspector history persists. |
| Solid `onCleanup` / Vue `onScopeDispose` | Same stack semantics — last-mount-wins, no duplicates. |
